...
int speed_map_1541[MAX_TRACKS_1541] = {
...
int capacity[] = { 0x1857, 0x19f6, 0x1bd1, 0x1df0 };
int capacity_min[] = { 0x1827, 0x19c6, 0x1ba1, 0x1dc0 };
int capacity_max[] = { 0x18a7, 0x1a46, 0x1c21, 0x1e90 };

/* Super Card defaults */
//int capacity[] = { 0x1848, 0x1a18, 0x1c08, 0x1e38 };
  ...
  int track, int sector, BYTE * id)
{
	...
	if (error_code == HEADER_NOT_FOUND)
	{
		...
		do
		{
			...
			convert_4bytes_from_GCR(gcr_ptr + 5, header + 4);
			gcr_ptr++;
		} while (header[0] == 0x07 || header[2] != sector ||
		  header[3] != track);
	}

	if (header[0] != 0x08)
		error_code = (error_code == SECTOR_OK) ?
		  HEADER_NOT_FOUND : error_code;
...
size_t
find_track_cycle(BYTE ** cycle_start, BYTE ** cycle_stop, int cap_min,
  int cap_max)
{
	...
	*cycle_stop = nib_track + GCR_TRACK_LENGTH;
	return GCR_TRACK_LENGTH;
}
...
check_formatted(BYTE * gcrdata)
{
	...
	{
		if (is_bad_gcr(gcrdata, GCR_TRACK_LENGTH, i))
			run = 0;
		else
			run++;
...
{
	BYTE work_buffer[GCR_TRACK_LENGTH*2];	/* working buffer */
	...
	memcpy(work_buffer, cycle_start, GCR_TRACK_LENGTH);

	/* find cycle */
	find_track_cycle(&cycle_start, &cycle_stop, cap_min - 150, cap_max);
	...
	else if (sector0_len != 0 || force_align == ALIGN_SEC0)
	{
		*align = ALIGN_SEC0;
...
int
check_errors(BYTE * gcrdata, int length, int track, BYTE * id,
  char * errorstring)
{
...
int
check_empty(BYTE * gcrdata, int length, int track, BYTE * id,
  char * errorstring)
{

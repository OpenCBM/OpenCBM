...
/*! ************************************************************** 
** \file lib/plugin/xuac/WINDOWS/install.c \n
...
static const char usagetext[] =
            "\n\nUsage: instcbm [options] xuac [plugin-options]\n"
            "Install the XUAC plugin and driver on the system, or remove it.\n"
            "\n"
...
static opencbm_plugin_install_neededfiles_t NeededFilesXUAC[] = 
{
    { SYSTEM_DIR, "opencbm-xuac.dll", NULL },
    { LIST_END,   "",                 NULL }
};
...
hint(void)
{
    fprintf(stderr, "Try \"instcbm xuac --help\" for more information.\n");
}
...
version(VOID)
{
    printf("opencbm xuac plugin version " /* OPENCBM_VERSION */ ", built on " __DATE__ " at " __TIME__ "\n");
}
...
opencbm_plugin_install_do_install(void * Context)
{
    ...
    DBG_PRINT((DBG_PREFIX "-- xuac.install" ));
...
opencbm_plugin_install_do_uninstall(void * Context)
{
    ...
    DBG_PRINT((DBG_PREFIX "-- xuac.uninstall" ));
...
{
    unsigned int size = sizeof(NeededFilesXUAC);
    xu1541_parameter_t *parameter = Data->OptionMemory;
    ...
    do {
        ...
        memcpy(Destination, NeededFilesXUAC, size);
